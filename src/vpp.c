/*
   ...
   Last update: <2010-06-16 16:30:52>
   */
...
inline static void taka_tvpp_certification(FILE *fp, const char *orig_file, const char cc)
{
    ...
    fprintf(fp, "\n\n%c%c\n", cc, cc);
    fprintf(fp, "%c%c Preprocessed by vpp-%s\n", cc, cc, VERSION);
    realpath(orig_file, rp);
    fprintf(fp, "%c%c for %s\n", cc, cc, rp);
    fprintf(fp, "%c%c by %s\n", cc, cc, getenv("USER"));
    fprintf(fp, "%c%c on %s\n", cc, cc, getenv("HOSTNAME"));
    time(&t);
...
static inline char * taka_tvpp_check_symbol(taka_tvpp_data_t *d, FILE *cerr)
{
    ...
    // normalize
    strcpy(d->n_line_str, d->line_str);
    taka_tstring_one_tab_space(d->n_line_str);

    // check comments
    if (*d->n_line_str != PPS && (p = strstr(d->line_str, COMMENTS_END))) {
        d->comment = FALSE;
        return (p);
    }
    if (d->comment) return (NULL);
    if (*d->n_line_str != PPS && (p = strstr(d->line_str, COMMENTS_START))) {
        d->comment = TRUE;
        return (p);
    }
...
int taka_tvpp_convert(const char *orig_file, const char *conv_file, tlist_t *def, const char cc, FILE *cerr)
{
    ...
    memset(&d, 0, sizeof (d));
    d.line_str = (char *)malloc(sizeof (char) * LINE_BUFFER_SIZE);
    d.n_line_str = (char *)malloc(sizeof (char) * LINE_BUFFER_SIZE);
    d.keyword = (char *)malloc(sizeof (char) * LINE_BUFFER_SIZE);
    d.label = (char *)malloc(sizeof (char) * LINE_BUFFER_SIZE);
    d.string = (char *)malloc(sizeof (char) * LINE_BUFFER_SIZE);
    d.including = (char *)malloc(sizeof (char) * FILENAME_MAX);
    *d.including = '\0';
    d.defined = def;
    ...
    for (;;) {
        memset(d.line_str,0,LINE_BUFFER_SIZE);
        if (!fgets(d.line_str, LINE_BUFFER_SIZE, of)) break;
        if (!(d.forstat.loop && d.forstat.begin != d.forstat.end)) d.line_num++;
        if (!feof(of) && !(p = strchr(d.line_str, '\n'))) {
        ...
        if (!cs && !d.ifstat && *d.n_line_str != PPS) {
            // replace
            tlist_for_reverse(l, d.replace) {
                taka_tstring_replace_str( &d.line_str,
                                          &n,
                                          ((taka_tvpp_str_pair_t *)l->data)->org,
                                          ((taka_tvpp_str_pair_t *)l->data)->chg );
            }
            ...
            tlist_for_reverse(l, d.rand) {
                ...
                taka_tstring_replace_str(&d.line_str, &n, ((taka_tvpp_str_pair_t *)l->data)->org, pRand);

                free(pRand);
            }
        }

        if (!(d.forstat.flag & 0x2)) fputs(d.line_str, cf);
        if (!feof(of)) fputc('\n', cf);
        ...
        if (*d.including != '\0') {
            ...
            incf = fopen(d.string, "rt");
            if (incf) {
                while (fgets(d.line_str, LINE_BUFFER_SIZE, incf)) fputs(d.line_str, cf);
                fclose(incf);
            } else {
...
inline static int taka_tvpp_create_module_template(const char *orig_file, FILE *cerr)
{
    ...
    cf = fopen(VPP_TEMPLATE_MODULE, "wt");
    if (!cf) {
        fclose(of);
        if (cerr) fprintf(cerr, "Can't open a converted file: %s.\n", VPP_TEMPLATE_MODULE);
        return (-EVPPMTCONV);
...
inline static int taka_tvpp_create_package_template(const char *orig_file, FILE *cerr)
{
    ...
    cf = fopen(VPP_TEMPLATE_PACKAGE, "wt");
    if (!cf) {
        fclose(of);
...
inline static int taka_tvpp_create_simulation_template(const char *orig_file, const char cc, FILE *cerr)
{
    ...
    cf = fopen(VPP_TEMPLATE_SIMULATION, "wt");
    if (!cf) {
        fclose(of);
